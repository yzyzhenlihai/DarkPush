...
**对于大端法和小端法的理解：** 所谓大端法和小端法，就是对于一段字节流，如果我们需要将多个字节进行组合（组合解析成整型或者其他数值型），就需要考虑大端和小端的问题，网络发过来的字节流都是按大端法的，如果在小端法的机器上，就会导致解析的顺序错误，所以我们需要对需要多字节解析的字节流在小端法的机器上进行**重新排列**。
## sock模型创建流程
