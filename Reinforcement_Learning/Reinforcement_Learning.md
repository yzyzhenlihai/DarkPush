...
**对于扩散模型构建分布的理解：** 扩散模型使用监督学习的训练机制，完成了对复杂数据概率分布的学习与表示。它不像传统映射（如 $\mathbf{z} \to \mathbf{x}$ 的 GANs 或 VAEs）那样直接从潜在空间映射到数据，而是通过建模分布的结构（即分布的梯度，利用去噪得分匹配，将复杂的概率学习问题转化为一个简单的监督学习的回归任务），来实现生成。

**在扩散模型的经典设定中**，固定方差是一种常见且有效的简化策略。通过将方差固定为理论上的后验方差值，神经网络 $f_\theta$ 只需要专注于学习高斯分布的**均值** $\mu_\theta$（即预测 $x_0$）。这意味着模型只需要学习“去噪的方向”，而不需要同时学习“去噪的不确定性幅度”，从而降低了训练难度并提高了稳定性。**在实际实现扩散模型时**，学习去噪网络，本质上就是学习分布的均值，已经隐含了数学公式中的固定方差。然后在具体推理是，通过固定的方差来进行反向去噪增加随机性。
...
#### POMDP
在MDP建模中，当下的观测的完全的，所以策略可以根据当前的观测做出动作。
但是对于POMDP，当下的观测是部分观测，单单根据这个部分观测，策略很难做出正确的观测，所以就有里历史（history）和信念（belief），历史就是策略交互的历史，而信念是根据之前的交互历史以及当前的部分观测得到的一个新状态，这个状态包含了过去的信息，能够让策略获得更多的信息进行决策。
**以推荐系统举例：**
1. 单点观测的局限性，POMDP 当下的观测是部分观测，单单根据这个部分观测，策略很难做出正确的决策，这在学术上叫做感知混叠（Perceptual Aliasing）。比如我们只是观测到了用户点击“不感兴趣”，认为用户讨厌这个视频，但是这个观测是不全面的，可能会有别的原因。
2. 历史交互，随着时间推移，$t$ 越来越大，历史列表会变得**无限长**，所以我们需要把无限长的历史压缩成固定大小的东西——信念
3. 信念构造，让问题回归马尔可夫。POMDP本身是非马尔可夫的，因为它依赖历史。但是如果把信念看作是新的状态，那么问题就又变回了一个MDP（Belief MDP）。
**充分统计量：** 信念是一个充分统计量，是一个统计学概念，一旦你拥有了这个统计量，原始数据就可以扔掉了，因为它包含了对你后续推断有用的所有信息。

#### 推荐系统评价指标
1. 累计增益（Cumulative Gain，CG）。CG 是搜索结果列表中所有文档的分级相关性得分的总和。CG 只考虑了搜索结果列表中文档的相关性，而**没有考虑**这些文档在结果列表中的**位置因素**。
	$$
	CG_p=\sum_{i=1}^prel_i
	$$
		其中 $rel_i$ 代表第 $i$ 位置上文档的相关度
2. 折损累计增益(Discounted cumulative gain，DCG）。DCG 提出在搜索结果列表的**较低位置**上出现**相关性较高**的文档时，应该对评测得分**施加惩罚**。惩罚比例与文档的所在位置的对数值相关。
	$$
...
3. 归一化折损累计增益（Normalized Discounted cumulative gain，NDCG）
	...
	$$
		其中 $IDCG_p$ 是理想情况下的 $DCG_P$ ，也就是所有相关性文档按最理想的顺序排序（相关性从小到大）。
	NDCG 的计算过程：系统会拿着 **“标准答案 $i_n$”** 去 **“预测列表”** 里找。如果 $i_n$ 出现在列表的第 1 位：恭喜，预测完美，NDCG 得分最高（接近 1）。
4. 平均倒数排名（Mean Reciprocal Rank, MRR）
	$$
	MRR=\frac{1}{Q}\sum^{\left|Q\right|}_{i=1}\frac{1}{rank_i}
	$$
		其中，$Q$ 是用户的个数，是对于第 $i$ 个用户，推荐列表中第一个在ground-truth结果中的item所在的排列位置。
### pytorch
