...
康托展开就是用来求解1~n的全排列中，任意排列的排名。要想要计算排名，就只需要计算有多少个比全排列x小的就可以了。

**示例：**

假设我们知道长度为5的排列[2，5，3，4，1]，逐位来讨论，第一位为2，大于以1为第一位的任何排列，个数为4！。对于第二位5而言，**它大于第一位与这个排列相同，第二位小于5的所有排列**（注意还要剔除前面已经出现过的数），因此第二位只能是1，3或4，那么数量为3×3！。同理可得，答案就是1+4！+3×3！+2！+1=46。注意我们统计的是排名，所以前面还要+1
...
1. 首先需要对无向边按照一定规则定向，设d[u]表示u节点的度数。如代码所示，此时这张图是一张**有向无环图**
