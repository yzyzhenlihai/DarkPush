...
RTMP(Real Time Messaging Protocol)，实时消息传输协议，默认使用端口1935。
RTMP协议有三个分支：
- 工作在TCP协议上的明文传输，它使用的端口是**1935**；
- RTMPT，RTMPT被封装在HTTP请求之中，可以穿越防火墙进行传输；
- RTMPS，它也是封装在HTTP之中，不过与RTMPT不同的是，它使用HTTPS安全连接，可以保证传输的安全。
- RTMP协议中基本的数据单元称为**消息（Message）**，即封装、解封装都是以Message为单位进行操作。当RTMP协议在互联网中传输数据的时候，为了更好地实现多路复用、分包和信息的公平性，发送端会把Message划分为**带有Message ID的Chunk**，每个Chunk可能是一个单独的Message，也可能是Message的一部分，在接受端会根据chunk中包含的data的长度，message id和message的长度把chunk还原成完整的Message，从而实现信息的收发；
##### 协议握手

- 一个RTMP连接以握手开始，先进行TCP握手后再进行RTMP握手。RTMP 握手由**三个固定长度**的块组成，有**简单握手和复杂握手**两种方式，两种握手方式信息流转的过程是相同的，只是消息中携带的信息不同。
- 握手实质上起到的是**验证的作用**，其中一项是会校验服务器，客户端的rtmp版本，如果版本兼容则可以收发数据，如果版本不兼容则说明不能收发数据，则握手会失败。
- 无论推流（直播）还是拉流（观看），都是客户端向服务端发起握手请求。第一条握手消息是客户端发送的；
- 客户端向服务端按序发送C0，C1，C2（按序）3个chunk，服务端向客户端按序发送S0，S1，S2（按序）3个chunk，然后才能进行有效信息的传输。RTMP协议并没有规定这6个Message的具体传输顺序，但需要保证以下几点：（简单握手和复杂握手均是如此）
    - 客户端要等收到S1之后才能发送C2；
...
这一点可以通过wireshark抓ffmpeg推流包进行验证：（简单握手和复杂握手均是如此） 
*/
｜client｜Server ｜
｜－－－C0 + C1－－－> |
｜<－－S0 + S1 + S2－－｜ 
｜－－－－ C2 －－－－>｜
```
1. c0和s1通常大小为1 byte的版本号，默认为0x03
2. c1和s1的长度固定为1536 byte。具体结构为
	- `timestamp(4 byte) + zero(4 byte) + random number(1528 byte)`
	- 在RTMP实践中，4 byte的时间戳对于握手阶段来说并不重要，重要的是后面的1528 byte的随机数。
3. RTMP协议规定，c2是s1的拷贝，s2是c1的拷贝。标准做法应该校验 C2 是否等于我刚才发的 S1，但为了兼容性某些不标准的推流器，很多服务器只校验长度
##### 块格式
...
**第一个字节低6bit值=1：** `Basic Header` 为3byte；fmt=2bit；此时第一个字节中后6bit的值为0，cs id=第2个byte，范围：64~65599（(第3个byte) * 256 + (第2个byte) + 64）。注意：第2个byte是CSID的低八位，第3个byte是CSID的高八位
...
**Message Header（块消息头）**
根据块基本头（basic header）中的 **fmt** 的值来区分块消息头，从0—3共4种，上图标的很明确了，具体如下：

**fmt = 0：** 块消息头为11字节，当前消息的 `timestamp` 在这表示( `Extended Timestamp` 辅助用)，如果用户设置时间戳>=0xFFFFFF时(3字节容不下了)时，`timestamp` 字段就固定为0xFFFFFF。`message length` 是指是整个message的大小。`Extended Timestamp` 字段用4字节表示；
...
**chunk stream id和message stream id的区别：** 
chunk stream id用来区分当前发送的chunk是属于控制数据、音频数据还是书视频数据。message stream id用来区分当前的chunk是属于哪一路流的。例如，假设当前chunk stream id=6表示正在传输视频数据，但是当时传输视频数据有两路流（一路是游戏画面，一路是直播画面），两路流的message stream id分别是3和4。**那对于CSID相同的情况，在拼接数据的时候，不同视频流的数据不会乱掉吗？**
答案是不会，RTMP会严格避免掉这种情况，只有在完整发送完一帧之后，才会发送另外一帧。或者还有另外一种解决思路，就是对于不同的MSID，将其分配到不同的CSID上传输。这种情况是针对同一条TCP连接，发送多路流，通过CSID来区分。【这种利用CSID区分还是在同一条TCP连接中】

**总结不同格式的使用场景：**
fmt0、fmt1、fmt2都表示新的一帧要开始，需要重新清空缓冲区接受新一帧的数据。fmt0和fmt1的区别在于fmt1少了message stream id，表示前后前后帧的message stream id是相同的，但是长度不同，所以需要在实际的代码实现时需要重新分配内存大小，但是fmt2只有时间增量，表示其他信息都和之前的帧相同。fmt3是用来传输同一个帧的后续分片，对于视频帧可能很大，所以需要分成多个chunk发送，所以不需要带时间戳（同一帧时间戳相同）。

**Extended Timestamp（扩展时间戳）**
当块消息头中的 `timestamp` 或者 `timestamp delta` 字段(3字节)容不下时(**fmt = 0，1或2**)，`Extended Timestamp` 才会被使用。

##### AMF命令消息
AMF（Action Message Format），利用AMF解码器，把二进制数据还原成字符串命令，然后根据当前的角色（服务端/客户端）以及命令的类型进行分发。
...
```
### 直播CDN
CDN (Content Delivery Network)，即内容分发网络，是一种网络架构，旨在提高用户对互联网上内容的访问速度和性能。CDN通过在全球各地部署大量的服务器节点，将内容缓存到离用户更近的服务器上，从而减少内容传输的距离，提高访问速度和响应时间。当用户请求访问某个网站或应用程序时，CDN会自动选择距离用户最近的服务器节点，从该节点提供内容，而不是直接从原始服务器获取内容。


## 一些提问
1.  为什么说RTMP更适合推流？
	- **浏览器的“封杀”**，过去浏览器里都有 Flash 插件，Flash 天生支持 RTMP，所以以前网页直播也用 RTMP，但是现代浏览器禁用了Flash，现代浏览器（HTML5 Video）原生不支持 RTMP。
	- **防火墙穿透能力差**，RTMP 使用 TCP 1935 端口。很多公司的内网防火墙、学校网管默认只开放 80 (HTTP) 和 443 (HTTPS) 端口，把 1935 这种“闲杂端口”封死了。如果你用 RTMP 做拉流，很多上班族在公司根本打不开视频。而HTTP-FLV/HLS：使用的是 80 端口，伪装成普通的网页请求，防火墙完全不会拦截。
	- **CDN分发成本与扩展性**，RTMP 是一个非常复杂的协议，中间维护了很多状态。如果在 CDN 边缘节点大规模分发 RTMP，对服务器的 CPU 和内存消耗较高。HTTP 协议简单，现有的 Web 服务器（Nginx 等）优化到了极致，分发 HTTP 静态文件（TS 切片）或者 HTTP 流（FLV）非常高效，且便宜。
...
if (0 == strcmp("/index.m3u8", uri)) {
	...
	sprintf(http_headers, "HTTP/1.1 200 OK\r\n"
		...
		"Content-Length: %d\r\n"  
	// application/vnd.apple.mpegurl是HLS索引文件的标准类型，告诉播放器这是一个播放列表
	...
	sprintf(http_headers, "HTTP/1.1 200 OK\r\n"
		"Access-Control-Allow-Origin: * \r\n"
	// 这里设置close，意味着发完切片就关闭连接，节省资源，但是现在HTTP1.1通常建议复用连接
		"Connection: close\r\n"     
		"Content-Length: %d\r\n"
	// 这是 MPEG-2 Transport Stream (TS) 的标准类型，即实际的视频数据
...
while (true) {
	...
	if (times == 1) {
		...
		//usleep(1000 * 100);
		// 模拟直播流发送，间隔一段时间发送5000字节的数据，不加sleep就成高速下载了
		// 真正的流媒体服务器需要根据视频的 DTS/PTS (时间戳) 来精确控制发送速度
		Sleep(10);
		int bufLen = fread(buf, 1, sizeof(buf), fp); // 每次读出buf大小的字节进行发送
...
2. RTMP协议的“拆包“封包”
	- 握手协议
	- AMF编解码，RTMP 的控制命令（如 `connect`, `publish`）是用 AMF0 或 AMF3 格式编码的。
	- Chunk解析
3. 数据转发核心
	- 流媒体源管理，主播推流注册Source，观众拉流找到Source
	- GOP缓存，这是“秒开”的关键。
	- 音视频配置头，对于SPS/PPS和AAC Spec的缓存机制
4. 进阶功能
	- 协议转换，是否支持RTMP转为HTTP-FLV
	- 内存优化，是否有Zero-copy，是memcpy还是引用计数
##### RTMP协议握手

**情况1：作为客户端/发布者角色**
```c++
// handshake_state表示当前的状态，表示执行到握手的哪一步
if (handshake_state_ == HANDSHAKE_S0S1S2) {
	if (buf_size < (1 + 1536 + 1536)) { //S0S1S2	
		// s0(1 byte) + s1(1536 byte) + s2(1536 byte) 如果数据不够则返回0继续等待
		return res_size;
	}
	// 检查s0版本号,通常是0x03
...
{	
	...
	else
	{
		...
		// s1结构 timestamp(4 byte) + zero(4 byte) + random number(1528 byte)
		// S1 的前 4 个字节应该是时间戳。这里直接 memset 为 0 了。
		// 这在 RTMP 实践中是允许的，因为握手阶段的时间戳通常不重要，重要的是后面的随机数。
		char* p = res_buf; p += 9;//跳过timestamp和zero
		// 按生成随机数填充s1的random number部分
		for (int i = 0; i < 1528; i++) {
			*p++ = rd();
		}
		// 生成s2，RTMP规定，s2必须是c1的拷贝
		memcpy(p, buf + 1, 1536);
		handshake_state_ = HANDSHAKE_C2; // 状态变更，等到接受c2
	}
}
```
**情况3：作为服务端角色，等待客户端c2握手包**
```c++
// 标准做法应该校验 C2 是否等于我刚才发的 S1，但为了兼容性某些不标准的推流器，很多服务器只校验长度
else if (handshake_state_ == HANDSHAKE_C2)
{
	if (buf_size < 1536) { //c2
		...
		pos = 1536;
		handshake_state_ = HANDSHAKE_COMPLETE;// 握手完成，可以开始发AMF命令了
	}
}
else {
	return -1;
...
##### 协议解析层
这个环节主要是实现 `chunk` 拼接成 `message` 是后续的业务逻辑处理的前提
##### 业务逻辑层
根据“协议解析层”拼接得到完整的 `message`后 ，根据不同的 `message` 类型执行不同的业务逻辑。

1. **处理推流命令**

   - Connect

   <img src="./images/picture22.png" alt="image-20251204153810205" style="zoom:50%;" />
   ...
   // 从客户端connect包中解析出来的Transaction ID，通常是1。类比RTSP的CSeq
   ...
   - releaseStream / FCPublish
     ...
     推流客户端可能会发送这两个命令，这是Flash时代的遗留，可以直接忽略，或者回复一个空的`_result`.

     ![image-20251204160821430](./images/picture23.png)

   - createStream

   - publish

     ![image-20251204171956360](./images/picture24.png)

2. **处理拉流命令**

#### 学习成果检测

1. **多路复用**：同一个 TCP 连接里，音频包和视频包是怎么交替传输且不乱序的？
2. **秒开**：为什么我刚连上就能看到画面，服务器做了什么？
3. **转发**：当主播推流断开时，服务器是如何通知所有正在观看的观众的？
...
**问题8：**出现下述错误
```c++
yyyyuzy@DESKTOP-J248F2V:~/code/RTSPServer$ ./bin/rtspserver 
[INFO]2025-11-23 09:53:47 [/home/yyyyuzy/code/RTSPServer/Live/Timer.cpp:83] timerfd=3
[INFO]2025-11-23 09:53:47 [/home/yyyyuzy/code/RTSPServer/Live/RtspServer.cpp:24] rtsp://0.0.0.0:1234
[INFO]2025-11-23 09:53:47 [/home/yyyyuzy/code/RTSPServer/main.cpp:17] ===== Session init start ====
[INFO]2025-11-23 09:53:47 [/home/yyyyuzy/code/RTSPServer/Live/H264FileMediaSource.cpp:18] Current Working Directory is /home/yyyyuzy/code/RTSPServer

[INFO]2025-11-23 09:53:48 [/home/yyyyuzy/code/RTSPServer/Live/H264FileSink.cpp:16] H264FileSink
[INFO]2025-11-23 09:53:48 [/home/yyyyuzy/code/RTSPServer/Live/AACFileSink.cpp:16] AACFileSink
[INFO]2025-11-23 09:53:48 [/home/yyyyuzy/code/RTSPServer/Live/RtspServer.cpp:60] clientFd=7 connect to RtspServer successfully
[ERROR]2025-11-23 09:53:48 [/home/yyyyuzy/code/RTSPServer/Live/RtspServer.cpp:55] RtspServer handleRead error,listenFd=4,clientFd=-1,errno = 9: Bad file descriptor
[INFO]2025-11-23 09:53:48 [/home/yyyyuzy/code/RTSPServer/Live/Buffer.cpp:76] 
Client=========>Server
OPTIONS rtsp://172.23.35.128:1234/test RTSP/1.0
CSeq: 1
User-Agent: Lavf61.7.100


[INFO]2025-11-23 09:53:48 [/home/yyyyuzy/code/RTSPServer/Live/RtspConnection.cpp:160] method: OPTIONS url: rtsp://172.23.35.128:1234/test version:RTSP/1.0

free(): invalid pointer
Aborted (core dumped)
```
解决方案：在使用`sscanf` 读取字符串时，格式字符串中的类型和实际接受的变量类型不一致，导致越界写，如下代码所示。在解析`port` 时，设置的接受类型是`%d`，那么`sscanf` 会严格执行向这个地址写入4字节，但是实际只有2字节是属于`port`的，而与`prot`相邻的是其他栈上的变量，此时剩余的2字节就会出现越界写。**修改方案就是严格匹配类型** 。
```c++
   //解析url中的资源名称
    unit16_t port = 0;
    char ip[64]={0};
...
    }
```
在 C/C++ 中，`scanf/sscanf/printf` 这类函数是**类型不安全**的。它们完全信任你给的格式字符串`%d`，哪怕你给错了指针类型，编译器往往也不会报错（除非开启了特定的警告），直到运行时才会引发内存破坏。
...
车企：蔚来、上汽大众、东风日产、汉德车桥
制造业：华测导航、中海达、潮汕三环
### 大佬的经验


1. 服务器框架就是一些网络库比如libuv、libevent、asio。客户端编码库就是ffmpeg的libav系列，流媒体协议就是rtmp、rtsp、hls这些，实时通信就是WebRTC。
2. 音视频开发SDK，熟悉开源音视频框架和开源库，如：ffmpeg、VLC、mplayer、obs等；
3. 音视频+模型部署。八股的话知道的面要广，核心知道就可以。项目要吃透（很重要），简历思维，面试官可能会问哪些问题。了解音视频未来会如何发展。
