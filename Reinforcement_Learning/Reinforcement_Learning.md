...
**对于扩撒模型构建分布的理解：** 扩散模型使用监督学习的训练机制，完成了对复杂数据概率分布的学习与表示。它不像传统映射（如 $\mathbf{z} \to \mathbf{x}$ 的 GANs 或 VAEs）那样直接从潜在空间映射到数据，而是通过建模分布的结构（即分布的梯度，利用去噪得分匹配，将复杂的概率学习问题转化为一个简单的监督学习的回归任务），来实现生成。
